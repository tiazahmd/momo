...
#define MOMO_VERSION "0.0.1"
#define MOMO_TAB_STOP 8
#define MOMO_QUIT_TIMES 3
...
char *c_HL_Keywords[] = {
      "switch", "if", "while", "for", "break", "continue", "return", "else",
        ...
        "#define", "int|", "long|", "double|", "float|", "char|", "unsigned|", "signed|",
        "void|", NULL
};
...
int editorRowCxToRx(erow *row, int cx) {
    ...
    for (j = 0; j < cx; j++) {
        if (row->chars[j] == '\t') {
            rx += (MOMO_TAB_STOP - 1) - (rx % MOMO_TAB_STOP);
        }
        rx++;
    }
    return rx;
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for (cx = 0; cx < row->size; cx++) {
        if (row->chars[cx] == '\t') {
            cur_rx += (MOMO_TAB_STOP - 1) - (cur_rx & MOMO_TAB_STOP);
        }
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);
    row->render = malloc (row->size + tabs*(MOMO_TAB_STOP - 1) + 1);

    int idx = 0;
    for (j = 0; j < row->size; j++) {
        if (row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while (idx % MOMO_TAB_STOP != 0) row->render[idx++] = ' ';
        }
        else {
...
void editorProcessKeypress() {
    static int quit_times = MOMO_QUIT_TIMES;
        ...
        if (filerow >= E.numrows) {
            if (E.numrows == 0 && y == E.screenrows / 3) {
                ...
                int welcomelen = snprintf(welcome, sizeof(welcome), 
                    "Momo Text Editor -- Version %s", MOMO_VERSION);
...
void editorDrawMessageBar(struct abuf *ab) {
    abAppend(ab, "\x1b[K", 3);
    int msglen = strlen(E.statusmesg);
    if (msglen > E.screencols) msglen = E.screencols;
    if (msglen && time(NULL) - E.statusmsg_time < 5) {
        abAppend(ab, E.statusmesg, msglen);
    }
}
...
void editorSetStatusMessage(const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(E.statusmesg, sizeof(E.statusmesg), fmt, ap);
    va_end(ap);
